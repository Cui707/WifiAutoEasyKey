...
3. **综合策略**:
   - 结合数字、字母、特殊字符的复杂组合
   - 常见密码模式（如"password123"、"admin123"等）
   - 用户可能设置的个性化密码

4. **高级分析建议**:
   注意尝试从该WIFI的名称中分析出该路由器的厂家，并通过网络查询该厂家路由器的默认密码。

请生成尽可能多的密码，不限字数、不限字符、不限组合方式，越多越好！

**返回格式要求**:
请以JSON格式返回分析结果，包含以下字段：
- "analysis": 路由器厂家分析建议
- "passwords": 密码列表（数组格式）
- "recommendations": 密码生成建议

返回的JSON格式示例：
```json
{
  "analysis": "根据SSID分析，可能是XX品牌路由器，默认密码为admin",
  "passwords": ["password1", "password2", "password3"],
  "recommendations": "建议尝试常见密码组合"
}
""";
}
